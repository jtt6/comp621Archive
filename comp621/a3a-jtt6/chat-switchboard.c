 ...
 * the connection over which the message arrived.
 */
		...
		struct conn_pool *p)
{
	...
	while (size != 0) {
		...
		if (pos != -1) {
			/* This message holds a complete line, finalize it. */
...
handle_new_connection(int listenfd, struct conn_pool *p)
{
	...
	/* We've handled a read from the listener FD, so there's one less
...
init_pool(int listenfd, struct conn_pool *p)
{
	...
	/* Initially, listenfd is the only member of the read descriptor set. */
	p->maxfd = listenfd;
	FD_ZERO(&p->read_set);
